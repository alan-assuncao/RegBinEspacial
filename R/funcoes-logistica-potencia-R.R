...
#' @export
 ...
 F <- function(x,delta) {
      z = (1/(1+exp(-x)))^(exp(delta))
      z
}

 #' @title Calculo da matriz de informacao de Fisher do modelo sob a distribuicao Logistica Potencia
#' @name Gpl
#'
#' @description Uma funcao que recebe os valores dos parametros, a matriz de delineamento X e a matriz de respostas Y
#'     e calcula a matriz de informacao de Fisher para o modelo probabilistico Logistica Potencia
#' @usage ${Gpl}(${theta, y, X})
#'
...
#'
#' @details The details of the Gpl function can be found in the vignette. Users can access the vignette using \verb{vignette(package = "SpatialBinReg")}.
#'     
#'
#' @return Uma matriz.
#'
...
#'
#'      
#'      Gpl(theta=c(delta,beta1,beta2,phi),y=Y,X=X)
#' 
#' @export
    ...
    # yis tem que entrar na forma vetorial (vetor de dimens�o n*m)
     ...
     # Informa��o de fisher de xi
        ...
        for (i in 1:m)
       {
           ...
           Lambdabetas =  diag(c(ddbeta*((1-pis)/pis)))
...
#' @usage ${lpostbetadeltaPL}(${theta,phiinit,y, X})
#'
...
#'
#'      
#'      lpostbetadelatPL(theta=c(delta,beta1,beta2),phiinit=phi,y=Y,X=X)
#' 
#' @export
...
#' @usage ${gradbetadeltaPL}(${theta,phiinit,y, X})
#'
#' @param theta Um vetor de parametros; \delta e o vetor de coeficientes \beta
#' @param phiinit Os valores dos efeitos aleatorios espaciais \phi_is , i=1,...,n e s=1,...,m
#' @param X Uma matriz de delineamento
...
#'
#'      
#'      gradbetadelatPL(theta=c(delta,beta1,beta2),phiinit=phi,y=Y,X=X)
#' 
#' @export
	...
	D=rep(0,(1+p))
	 ...
	 D[1] = sum(grad)
        ...
        for (i in 1:m)
       {
           ...
           omegas = dddeltabeta*((y[,i]-pis)/pis) # s = 1, ...,m

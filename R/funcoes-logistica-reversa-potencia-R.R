...
#' @export
 ...
 F <- function(x,delta) {
      z = 1-(1/(1+exp(x)))^(exp(delta))
      z
}

#' @title Calculo da matriz de informacao de Fisher do modelo sob a distribuicao Logistica Reversa de Potencia 
#' @name Grpl
#'
#' @description Uma funcao que recebe os valores dos parametros, a matriz de delineamento X e a matriz de respostas Y
#'     e calcula a matriz de informacao de Fisher para o modelo probabilistico Logistica Reversa de Potencia 
#' @usage ${Grpl}(${theta, y, X})
#'
...
#'
#' @details The details of the Grpl function can be found in the vignette. Users can access the vignette using \verb{vignette(package = "SpatialBinReg")}.
#'     
#'
#' @return Uma matriz.
#'
...
#'
#'      
#'      Grpl(theta=c(delta,beta1,beta2,phi),y=Y,X=X)
#' 
#' @export
    ...
    # yis tem que entrar na forma vetorial (vetor de dimens�o n*m)
     ...
     # Informa��o de fisher de xi
       ...
       for (i in 1:m)
       {
           ...
           graddeltas = (exp(delta)*log(1+exp(zis)))/((1+exp(zis))^(exp(delta))-1)
        ...
        for (i in 1:m)
       {
           ...
           ddbeta=c(exp(2*(delta+zis))/(((1+exp(zis))^(exp(delta))-1)*dd)^2)
           
           Lambdabetas =  diag(c(ddbeta*(pis/(1-pis))))
            
          Inffisherbeta = Inffisherbeta + t(X)%*%Lambdabetas%*%X
        }
          #    print(Inffisherbeta)
        # Informa��o de fisher de delta e beta
        ...
        for (i in 1:m)
       {
           ...
           grads = (exp(delta)*log(1+exp(zis)))/((1+exp(zis))^(exp(delta))-1)  
           
           dd=(1+exp(zis))

           deltabeta = c(exp(delta+zis)/(((1+exp(zis))^(exp(delta))-1)*dd))
...
#' @usage ${lpostbetadeltaRPL}(${theta,phiinit,y, X})
#'
...
#'
#'      
#'      lpostbetadelatRPL(theta=c(delta,beta1,beta2),phiinit=phi,y=Y,X=X)
#' 
#' @export
...
#	ll=ifelse(lpostbetadelta!='-Inf',lpostbetadelta,-10000000000000000000)
  	ll=ifelse(any(lpostbetadelta=='-Inf' | lpostbetadelta =='NaN'),-10000000000000000000,lpostbetadelta)
...
#' @usage ${gradbetadeltaRPL}(${theta,phiinit,y, X})
#'
#' @param theta Um vetor de parametros; \delta e o vetor de coeficientes \beta
#' @param phiinit Os valores dos efeitos aleatorios espaciais \phi_is , i=1,...,n e s=1,...,m
#' @param X Uma matriz de delineamento
...
#'
#'      
#'      gradbetadelatRPL(theta=c(delta,beta1,beta2),phiinit=phi,y=Y,X=X)
#' 
#' @export
...
#  Sigma=solve(Omega)             # phi ~ N(0, Sigma), onde Sigma = Omega^-1 
	...
	D=rep(0,(1+p))
       ...
       for (i in 1:n)
       {
           ...
           deltais = (exp(delta)*log(1+exp(zis)))/((1+exp(zis))^(exp(delta))-1)
	 ...
	 D[1] = sum(grad)
        ...
        for (i in 1:m)
       {
           ...
           dddeltabeta=exp(delta+zis)/(((1+exp(zis))^(exp(delta))-1)*(1+exp(zis)))

           omegas = dddeltabeta*((y[,i]-pis)/(1-pis)) # s = 1, ...,m

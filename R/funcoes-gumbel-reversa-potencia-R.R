...
#' @export
 ...
 F <- function(x,delta) {
      z = (1-exp(-exp(x)))^(exp(delta))
      z
}

#' @title Calculo da matriz de informacao de Fisher do modelo sob a distribuicao Gumbel Reversa Potencia
#' @name Gprg
#'
#' @description Uma funcao que recebe os valores dos parametros, a matriz de delineamento X e a matriz de respostas Y
#'     e calcula a matriz de informacao de Fisher para o modelo probabilistico Gumbel Reversa Potencia 
#' @usage ${Gprg}(${theta, y, X})
#'
...
#'
#' @details The details of the Gprg function can be found in the vignette. Users can access the vignette using \verb{vignette(package = "SpatialBinReg")}.
#'     
#'
#' @return Uma matriz.
#'
...
#'
#'      
#'      Gprg(theta=c(delta,beta1,beta2,phi),y=Y,X=X)
#' 
#' @export
    ...
    # yis tem que entrar na forma vetorial (vetor de dimens�o n*m)
     ...
     # Informa��o de fisher de xi
       ...
       for (i in 1:m)
       {
           ...
           graddeltas = log(1-exp(-exp(zis)))*exp(delta)
        ...
        for (i in 1:m)
       {
           ...
           grads = log(1-exp(-exp(zis)))*exp(delta)
...
#' @title Calculo da log-posteriori dos parametros \delta e \beta sob o modelo Gumbel Reversa Potencia
...
#' @usage ${lpostbetadeltaPRG}(${theta,phiinit,y, X})
#'
...
#'
#'      
#'      lpostbetadelatPRG(theta=c(delta,beta1,beta2),phiinit=phi,y=Y,X=X)
#' 
#' @export
...
#'
#'      
#'      gradbetadelatPRG(theta=c(delta,beta1,beta2),phiinit=phi,y=Y,X=X)
#' 
#' @export
	...
	D=rep(0,(1+p))
	 ...
	 D[1] = sum(grad)
        ...
        for (i in 1:m)
       {
           ...
           omegas = (exp(delta)*exp(zis-exp(zis)))/(1-exp(-exp(zis)))*((y[,i]-pis)/(1-pis)) # s = 1, ...,m

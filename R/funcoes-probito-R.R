#' @title Calculo da matriz de informacao de Fisher do modelo sob a funcao de ligacao Probito
#' @name Gprobit
#'
#' @description Uma funcao que recebe os valores dos parametros, a matriz de delineamento X e a matriz de respostas Y
#'     e calcula a matriz de informacao de Fisher para a funcao de ligacao Probito
#' @usage ${Gprobit}(${theta, y, X})
#'
...
#'
#' @details The details of the Gprobit function can be found in the vignette. Users can access the vignette using \verb{vignette(package = "SpatialBinReg")}.
#'     
#'
#' @return Uma matriz.
#'
...
#'        
#'        p = pnorm((betax[k]+phi[k,]))
#'        
#'        Y[k,] = rbinom(m, size=1, prob=p)
#'      }
#'
#'      
#'      Gprobit(theta=c(beta1,beta2,phi),y=Y,X=X)
#' 
#' @export
...
G = function(theta,y, X) {
     ...
     p = length(X[1,])  # quantidade de covariaveis
        ...
        for (i in 1:m)
       {
           ...
           dzis = dnorm(zis,mean=0,sd=1) # densidade da normal 0-1
           
           pis = F(zis)
           
           Lambdabetas =  diag(c(dzis^(2)/(F(zis)*F(-zis))))
...
#' @usage ${lpostbetadeltaProbit}(${theta,phiinit,y, X})
#'
...
#'
#'      
#'      lpostbetadelatProbit(theta=c(delta,beta1,beta2),phiinit=phi,y=Y,X=X)
#' 
#' @export
...
#' @usage ${gradbetadeltaProbit}(${theta,phiinit,y, X})
#'

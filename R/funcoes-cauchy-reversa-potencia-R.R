...
#' @export
 ...
 F <- function(x,delta) {
      z = 1-((1/pi)*atan(-x)+0.5)^(exp(delta))
      z
}

 #' @title Calculo da matriz de informacao de Fisher do modelo sob a distribuicao Cauchy Reversa de Potencia
#' @name Grpc
#'
#' @description Uma funcao que recebe os valores dos parametros, a matriz de delineamento X e a matriz de respostas Y
#'     e calcula a matriz de informacao de Fisher para o modelo probabilistico Cauchy Reversa de Potencia 
#' @usage ${Gpc}(${theta, y, X})
#'
...
#'
#' @details The details of the Grpc function can be found in the vignette. Users can access the vignette using \verb{vignette(package = "SpatialBinReg")}.
#'     
#'
#' @return Uma matriz.
#'
...
#'
#'      
#'      Grpc(theta=c(delta,beta1,beta2,phi),y=Y,X=X)
#' 
#' @export
    ...
    # yis tem que entrar na forma vetorial (vetor de dimens�o n*m)
     ...
     # Informa��o de fisher de xi
       ...
       for (i in 1:m)
       {
           ...
           graddeltas = exp(delta)*(log(2*pi)-log(2*atan(-zis)+pi))
        ...
        for (i in 1:m)
       {
           ...
           ddbeta=c(2*exp(delta)/((2*atan(-zis)+pi)*(1+zis^2)))
        ...
        for (i in 1:m)
       {
           ...
           grads = exp(delta)*(log(2*pi)-log(2*atan(-zis)+pi))
           
           deltabeta = 2*exp(delta)/((2*atan(-zis)+pi)*(1+zis^2))
...
#' @usage ${lpostbetadelta}(${theta,phiinit,y, X})
#'
...
#'
#'      
#'      lpostbetadelatRPC(theta=c(delta,beta1,beta2),phiinit=phi,y=Y,X=X)
#' 
#' @export
...
#' @usage ${gradbetadelta}(${theta,phiinit,y, X})
#'
#' @param theta Um vetor de parametros; \delta e o vetor de coeficientes \beta
#' @param phiinit Os valores dos efeitos aleatorios espaciais \phi_is , i=1,...,n e s=1,...,m
#' @param X Uma matriz de delineamento
...
#'
#'      
#'      gradbetadelatRPC(theta=c(delta,beta1,beta2),phiinit=phi,y=Y,X=X)
#' 
#' @export
	...
	D=rep(0,(1+p))
	 ...
	 D[1] = sum(grad)
        ...
        for (i in 1:m)
       {
           ...
           omegas = 2*exp(delta)/((2*atan(-zis)+pi)*(1+zis^2))*((y[,i]-pis)/pis) # s = 1, ...,m

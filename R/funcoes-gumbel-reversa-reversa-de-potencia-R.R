...
#' @export
 ...
 F <- function(x,delta) {
      z = 1-(1-exp(-exp(-x)))^(exp(delta))
      z
}

#' @title Calculo da matriz de informacao de Fisher do modelo sob a distribuicao Gumbel Reversa Reversa de Potencia
#' @name Grprg
#'
#' @description Uma funcao que recebe os valores dos parametros, a matriz de delineamento X e a matriz de respostas Y
#'     e calcula a matriz de informacao de Fisher para o modelo probabilistico Gumbel Reversa Reversa de Potencia 
#' @usage ${Grprg}(${theta, y, X})
#'
...
#'
#' @details The details of the Grprg function can be found in the vignette. Users can access the vignette using \verb{vignette(package = "SpatialBinReg")}.
#'     
#'
#' @return Uma matriz.
#'
...
#'
#'      
#'      Grprg(theta=c(delta,beta1,beta2,phi),y=Y,X=X)
#' 
#' @export
    ...
    # yis tem que entrar na forma vetorial (vetor de dimens�o n*m)
     ...
     # Informa��o de fisher de xi
       ...
       for (i in 1:m)
       {
           ...
           graddeltas = -exp(delta)*log(1-exp(-exp(-zis)))
           
           Lambdadeltas =  diag(c((1-pis)/pis))
        ...
        for (i in 1:m)
       {
           ...
           ddbeta=c((exp(delta)*exp(-zis-exp(-zis)))/(1-exp(-exp(-zis))))
           
           Lambdabetas =  diag(c(ddbeta^2*((1-pis)/pis)))
        ...
        for (i in 1:m)
       {
           ...
           grads = -exp(delta)*log(1-exp(-exp(-zis)))
           
           deltabeta = (exp(delta)*exp(-zis-exp(-zis)))/(1-exp(-exp(-zis)))
           
           Lambdadeltabetas =  diag(c(deltabeta*((1-pis)/pis)))
...
#' @usage ${lpostbetadeltaRPRG}(${theta,phiinit,y, X})
#'
...
#'
#'      
#'      lpostbetadelatRPRG(theta=c(delta,beta1,beta2),phiinit=phi,y=Y,X=X)
#' 
#' @export
...
#	ll=ifelse(any(lpostbetadelta!='-Inf' | lpostbetadelta !='NaN'),lpostbetadelta,-10000000000000000000)
...
#' @usage ${gradbetadeltaPRG}(${theta,phiinit,y, X})
#'
#' @param theta Um vetor de parametros; \delta e o vetor de coeficientes \beta
#' @param phiinit Os valores dos efeitos aleatorios espaciais \phi_is , i=1,...,n e s=1,...,m
#' @param X Uma matriz de delineamento
...
#'
#'      
#'      gradbetadelatRPRG(theta=c(delta,beta1,beta2),phiinit=phi,y=Y,X=X)
#' 
#' @export
...
#  Sigma=solve(Omega)             # phi ~ N(0, Sigma), onde Sigma = Omega^-1 
	...
	D=rep(0,(1+p))
       ...
       for (i in 1:n)
       {
           ...
           pis = F(zis,delta)
                                                # s = 1, ...,m
           grad[i] = sum(deltais*((y[i,]-pis)/pis))
        }
	 ...
	 D[1] = sum(grad)
        ...
        for (i in 1:m)
       {
           ...
           omegas = ((exp(delta)*exp(-zis-exp(-zis)))/(1-exp(-exp(-zis))))*((y[,i]-pis)/pis) # s = 1, ...,m

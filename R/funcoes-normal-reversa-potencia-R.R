...
#' @export
 ...
 F <- function(x,delta) {
      z = 1-(pnorm(-x))^(exp(delta))
      z
}

#' @title Calculo da matriz de informacao de Fisher do modelo sob a distribuicao Normal Reversa de Potencia
#' @name Grpn
#'
#' @description Uma funcao que recebe os valores dos parametros, a matriz de delineamento X e a matriz de respostas Y
#'     e calcula a matriz de informacao de Fisher para o modelo probabilistico Normal Reversa de Potencia
#' @usage ${Grpn}(${theta, y, X})
#'
...
#'
#' @details The details of the Grpn function can be found in the vignette. Users can access the vignette using \verb{vignette(package = "SpatialBinReg")}.
#'     
#'
#' @return Uma matriz.
#'
...
#'
#'      
#'      Grpn(theta=c(delta,beta1,beta2,phi),y=Y,X=X)
#' 
#' @export
...
#' @usage ${lpostbetadeltaRPN}(${theta,phiinit,y, X})
#'
...
#'
#'      
#'      lpostbetadelatRPN(theta=c(delta,beta1,beta2),phiinit=phi,y=Y,X=X)
#' 
#' @export
...
#' @usage ${gradbetadeltaRPN}(${theta,phiinit,y, X})
#'
#' @param theta Um vetor de parametros; \delta e o vetor de coeficientes \beta
#' @param phiinit Os valores dos efeitos aleatorios espaciais \phi_is , i=1,...,n e s=1,...,m
#' @param X Uma matriz de delineamento
...
#'
#'      
#'      gradbetadelatRPN(theta=c(delta,beta1,beta2),phiinit=phi,y=Y,X=X)
#' 
#' @export
	...
	D=rep(0,(1+p))
	 ...
	 D[1] = sum(grad)
        ...
        for (i in 1:m)
       {
           ...
           omegas = (exp(delta)*dnorm(-zis)/pnorm(-zis))*((y[,i]-pis)/pis) # s = 1, ...,m

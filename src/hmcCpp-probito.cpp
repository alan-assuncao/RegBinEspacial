...
//' @title Calculo da funcao de distribuicao acumulada da Normal padrao (funcao de ligacao Probito)
//' @name Fprobit
//'
//' @description Uma funcao que recebe dois valores, o valor de x e o valor do parametro de forma delta
//'     e calcula o valor da funcao de distribuicao acumulada do modelo probabilistico Normal padrao (funcao de ligacao Probito)
//'
//' @usage ${Fprobit}(${x})

//' @param x Um numero real
//'
//' @details The details of the Fprobit function can be found in the vignette. Users can access the vignette using \verb{vignette(package = "SpatialBinReg")}.
//'
...
//' @title Calculo da log-posteriori dos coeficientes de regressao \beta sob a funcao de ligacao Probito
//' @name lpostbetadeltaProbit
//'
...
//'     e calcula a log-posteriori para os oeficientes de regressao \beta sob a funcao de ligacao Probito
//' @usage ${lpostbetadeltaProbit}(${theta,y, X})
//'
//' @param theta Um vetor de parametros; a saber, vetor de coeficientes \beta e os valores dos efeitos aleatorios espaciais \phi_is , i=1,...,n e s=1,...,m
//' @param X Uma matriz de delineamento
//' @param Y Uma matriz de respostas binarias
//'
//' @details The details of the lpostbetadeltaProbit function can be found in the vignette. Users can access the vignette using \verb{vignette(package = "SpatialBinReg")}.
//'     
//'
...
//'        
//'        p = Fprobit((betax[k]+phi[k,]))
//'        
//'        Y[k,] = rbinom(m, size=1, prob=p)
//'      }
//'
//'      
//'      lpostbetadelatProbit(theta=c(beta1,beta2,phi),y=Y,X=X)
//' 
// [[Rcpp::export]]
...
//' @title Calculo do gradiente da log-posteriori dos coeficientes de regressao \beta sob a funcao de ligacao Probito
//' @name gradbetadeltaProbit
//'
...
//' @usage ${gradbetadeltaProbit}(${theta,y, X})
//'
...
//' @param Y Uma matriz de respostas binarias
//'
//' @details The details of the gradbetadeltaProbit function can be found in the vignette. Users can access the vignette using \verb{vignette(package = "SpatialBinReg")}.
//'     
//'
//' @return Um vetor.
//'
...
//'        phi[k,] = mvrnorm(1, mu = rep(0, times = m), Sigma = solve(Omega_sim))
//'        
//'        p = Fprobit((betax[k]+phi[k,]),delta)
//'        
//'        Y[k,] = rbinom(m, size=1, prob=p)
//'      }
//'
//'      
//'      gradbetadelatProbit(theta=c(beta1,beta2,phi),y=Y,X=X)
//' 
// [[Rcpp::export]]
...
arma::vec gradbetaCpp(arma::vec theta,arma::mat y,arma::mat X) {
    ...
    arma::colvec omegas;
        ...
        for (int i=0; i<m; i++)
       {
         ...
         dzis = arma::normpdf(zis);
         pis = Fcpp(zis);
           
         omegas = ((y.col(i)-pis)/(pis%(1-pis)))%dzis; // s = 1, ...,m
...
//' @title Calculo da log-posteriori do vetor de efeitos aleatorios \phi_i, para i=1,...,n sob a funcao de ligacao Probito
//' @name lpostphiProbit
//'
...
//'     e calcula a log-posteriori para o vetor de efeitos aleatorios sob o modelo com funcao de ligacao Probito
//' @usage ${lpostphiProbit}(${phi, bet, y, X, Omega})
//'
...
//'
//' @details The details of the lpostphiProbit function can be found in the vignette. Users can access the vignette using \verb{vignette(package = "SpatialBinReg")}.
//'     
//'
//' @return Um valor real.
//'
...
//'
//'      
//'      lpostphiProbit(phi=phi[1,],bet=c(beta1,beta2),y=Y[1,],X=X[1,])
//' 
// [[Rcpp::export]]
...
//' @title Calculo do gradiente da log-posteriori do vetor de efeitos aleatorios \phi_i, para i=1,...,n para o modelo sob funcao de ligacao Probito
//' @name gradphiProbit
//'
...
//' e calcula o gradiente da log-posteriori sob o modelo com funcao de ligacao Probito 
//' @usage ${gradphiProbit}(${phi, bet, y, X, Omega})
//'
//' @param phi O vetor de efeitos aleatorios espaciais \phi_i , i=1,...,n 
//' @param bet O vetor de coeficientes \beta
//' @param y Um vetor com respostas binarias para a unidade observacional i, para i=1,...,n
...
//' @param Omega Uma matriz referente à matriz de precisao
//'
//' @details The details of the gradphiProbit function can be found in the vignette. Users can access the vignette using \verb{vignette(package = "SpatialBinReg")}.
//'     
//'
//' @return Um vetor.
//'
...
//'
//'      
//'      gradphiProbit(phi=phi[1,],bet=c(beta1,beta2),y=Y[1,],X=X[1,])
//' 
// [[Rcpp::export]]
   ...
   arma::vec DD=-1000000000000*arma::ones(m);
        ...
        for (int j=0; j<m; j++){
         ...
         pis = Fcpp(zis);
          ...
          gradphiis = ((y/pis)-((1-y)/(1-pis)))%dzis;
...
//' e dos coeficientes de regressao \beta para o modelo sob funcao de ligacao Probito
//' @name hmcProbit
//'
...
//' para o modelo sob funcao de ligacao Probito
//' @usage ${hmcProbit}(${theta_current, SS,burn,lag,epsilon,LF, epsphi, Lphi M, y, X, Omega})
//'
...
//'
//' @details The details of the hmcProbit function can be found in the vignette. Users can access the vignette using \verb{vignette(package = "SpatialBinReg")}.
//'     
//'
//' @return Um vetor.
//'
...
//' map = optim(para, lpostbetadeltaProbit, gradbetadeltaProbit,phi, y = Y, X=X, control = list(fnscale = -1), method = 'BFGS', hessian = TRUE); map
//'
//'
//' G. = Gprobit(theta=c(round(map$par,2),phi),y=Y,X=X)
//'
...
//' Omegacpp=Omega_sim
//'
//'  B=hmcProbit(theta[i-1,],SS=1,burn=1,lag=1, epsilon=0.1, LF=22,epsphi = 0.001, Lphi = 22, M=G., y=Y,X=X, Omega=Omegacpp)
//' 
// [[Rcpp::export]]

...
//' @title Calculo da funcao de distribuicao acumulada da distribuicao Gumbel Reversa Reversa de Potencia
//' @name Frprg
//'
//' @description Uma funcao que recebe dois valores, o valor de x e o valor do parametro de forma delta
//'     e calcula o valor da funcao de distribuicao acumulada para o modelo probabilistico Gumbel Reversa Reversa de Potencia 
//'
//' @usage ${Frprg}(${x, delta})
...
//'
//' @details The details of the Frprg function can be found in the vignette. Users can access the vignette using \verb{vignette(package = "SpatialBinReg")}.
//'
...
arma::vec Fcpp (arma::vec x, double delta) { 
   ...
   int n = x.n_elem;
     ...
     z = 1-pow((1-exp(-exp(-x))),exp(delta));
      
    return z;    
}

//' @title Calculo da log-posteriori dos parametros \delta e \beta sob o modelo Gumbel Reversa Reversa de Potencia
//' @name lpostbetadeltaRPRG
//'
...
//'     e calcula a log-posteriori para os parametros \delta e \beta sob o modelo probabilistico Gumbel Reversa Reversa de Potencia 
//' @usage ${lpostbetadeltaRPRG}(${theta,y, X})
//'
...
//'
//' @details The details of the lpostbetadeltaRPRG function can be found in the vignette. Users can access the vignette using \verb{vignette(package = "SpatialBinReg")}.
//'     
//'
...
//'        
//'        p = Frprg((betax[k]+phi[k,]),delta)
//'        
//'        Y[k,] = rbinom(m, size=1, prob=p)
//'      }
//'
//'      
//'      lpostbetadelatRPRG(theta=c(delta,beta1,beta2,phi),y=Y,X=X)
//' 
// [[Rcpp::export]]
...
//' @title Calculo do gradiente da log-posteriori dos parametros (\delta, \beta) sob o modelo Gumbel Reversa Reversa de Potencia
//' @name gradbetadeltaRPRG
//'
//' @description Uma funcao que recebe os valores dos parametros \delta (parametro de forma) e coeficientes da regressao (\beta); a matriz de delineamento X e a matriz de respostas Y;
//' os valores dos efeitos aleatorios espaciais \phi_{is} e calcula o gradiente da log-posteriori sob o modelo probabilistico Gumbel Reversa Reversa de Potencia 
//' @usage ${gradbetadeltaRPRG}(${theta,y, X})
//'
...
//' @param Y Uma matriz de respostas binarias
//'
//' @details The details of the gradbetadeltaRPRG function can be found in the vignette. Users can access the vignette using \verb{vignette(package = "SpatialBinReg")}.
//'     
//'
//' @return Um vetor.
//'
...
//'
//'      
//'      gradbetadelatRPRG(theta=c(delta,beta1,beta2,phi),y=Y,X=X)
//' 
//'
// [[Rcpp::export]]
...
arma::vec gradbetadeltaCpp(arma::vec theta,arma::mat y,arma::mat X) {
  ...
  arma::vec phisoma(m);
       ...
       {             
           ...
           deltais = -exp(delta)*log(1-exp(-exp(-zis)));
           
          // arma::vec pis = 1-exp(-pow(zis,(-1/xi)));
           arma::vec pis = Fcpp(zis,delta);
                                                // s = 1, ...,m
           grad[i] = sum(deltais%((y.row(i).t()-pis)/pis)); 
    ...
    arma::colvec omegas;
       ...
       //  pis = 1-exp(-pow(zis2,(-1/xi)));
        pis = Fcpp(zis2,delta);
         ...
         omegas = ((exp(delta)*exp(-zis2-exp(-zis2)))/(1-exp(-exp(-zis2))))%((y.col(i)-pis)/pis); // s = 1, ...,m
...
//' @title Calculo da log-posteriori do vetor de efeitos aleatorios \phi_i, para i=1,...,n sob o modelo Gumbel Reversa Reversa de Potencia
//' @name lpostphiRPRG
//'
...
//'     e calcula a log-posteriori para o vetor de efeitos aleatorios sob o modelo probabilistico Gumbel Reversa Reversa de Potencia 
//' @usage ${lpostphiRPRG}(${phi,delta, bet, y, X, Omega})
//'
...
//'
//' @details The details of the lpostphiRPRG function can be found in the vignette. Users can access the vignette using \verb{vignette(package = "SpatialBinReg")}.
//'     
//'
//' @return Um valor real.
//'
...
//'
//'      
//'      lpostphiRPRG(phi=phi[1,],delta=delta,bet=c(beta1,beta2),y=Y[1,],X=X[1,])
//' 
//'
// [[Rcpp::export]]
...
//' @title Calculo do gradiente da log-posteriori do vetor de efeitos aleatorios \phi_i, para i=1,...,n sob o modelo Gumbel Reversa Reversa de Potencia
//' @name gradphiRPRG
//'
//' @description Uma funcao que recebe os valores do vetor de efeitos aleatorios espaciais, dos parametros \delta (parametro de forma) 
//' e coeficientes da regressao (\beta); a matriz de delineamento X e a matriz de respostas Y; e matriz de precisao \Omega
//' e calcula o gradiente da log-posteriori sob o modelo probabilistico Gumbel Reversa Reversa de Potencia 
//' @usage ${gradphiRPRG}(${phi,delta, bet, y, X, Omega})
//'
//' @param phi O vetor de efeitos aleatorios espaciais \phi_i , i=1,...,n 
//' @param delta O valor do parametro de forma \delta 
//' @param bet O vetor de coeficientes \beta
...
//' @param Omega Uma matriz referente à matriz de precisao
//'
//' @details The details of the gradphiRPRG function can be found in the vignette. Users can access the vignette using \verb{vignette(package = "SpatialBinReg")}.
//'     
//'
//' @return Um vetor.
//'
...
//'
//'      
//'      gradphiRPRG(phi=phi[1,],delta=delta,bet=c(beta1,beta2),y=Y[1,],X=X[1,])
//' 
// [[Rcpp::export]]
...
arma::vec gradphiCpp(arma::vec phi, double delta, arma::vec bet, arma::vec y, arma::vec X, arma::mat Omega) {
  ...
  arma::vec D;
        ...
        for (int j=0; j<m; j++){
         ...
         dzis = ((exp(delta)*exp(-zis-exp(-zis)))/(1-exp(-exp(-zis))));
           
         gradphiis = dzis%((y-pis)/pis);
...
//' e dos coeficientes de regressao \beta sob o modelo Gumbel Reversa Reversa de Potencia
//' @name hmcRPRG
//'
...
//' o modelo Gumbel Reversa Reversa de Potencia
//' @usage ${hmcRPRG}(${theta_current, SS,burn,lag,epsilon,LF, epsphi, Lphi M, y, X, Omega})
//'
...
//'
//' @details The details of the hmcRPRG function can be found in the vignette. Users can access the vignette using \verb{vignette(package = "SpatialBinReg")}.
//'     
//'
//' @return Um vetor.
//'
...
//' map = optim(para, lpostbetadeltaRPRG, gradbetadeltaRPRG,phi, y = Y, X=X, control = list(fnscale = -1), method = 'BFGS', hessian = TRUE); map
//'
//'
//' G. = Grprg(theta=c(round(map$par,2),phi),y=Y,X=X)
//'
...
//' Omegacpp=Omega_sim
//'
//'  B=hmcRPRG(theta[i-1,],SS=1,burn=1,lag=1, epsilon=0.1, LF=22,epsphi = 0.001, Lphi = 22, M=G., y=Y,X=X, Omega=Omegacpp)
//' 
// [[Rcpp::export]]

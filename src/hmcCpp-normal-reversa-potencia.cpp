...
//' @title Calculo da funcao de distribuicao acumulada da distribuicao Normal Reversa de Potencia
//' @name Frpn
//'
//' @description Uma funcao que recebe dois valores, o valor de x e o valor do parametro de forma delta
//'     e calcula o valor da funcao de distribuicao acumulada para o modelo probabilistico Normal Reversa de Potencia 
//'
//' @usage ${Frpn}(${x, delta})
...
//'
//' @details The details of the Frpn function can be found in the vignette. Users can access the vignette using \verb{vignette(package = "SpatialBinReg")}.
//'
...
arma::vec Fcpp (arma::vec x, double delta) { 
      ...
      z = 1-pow(normcdf(-x),exp(delta));
      
    return z;    
}

//' @title Calculo da log-posteriori dos parametros \delta e \beta sob o modelo Normal Reversa de Potencia
//' @name lpostbetadeltaRPN
//'
...
//'     e calcula a log-posteriori para os parametros \delta e \beta sob o modelo probabilistico Normal Reversa de Potencia 
//' @usage ${lpostbetadeltaRPN}(${theta,y, X})
//'
...
//'
//' @details The details of the lpostbetadeltaRPN function can be found in the vignette. Users can access the vignette using \verb{vignette(package = "SpatialBinReg")}.
//'     
//'
...
//'        
//'        p = Frpn((betax[k]+phi[k,]),delta)
//'        
//'        Y[k,] = rbinom(m, size=1, prob=p)
//'      }
//'
//'      
//'      lpostbetadelatRPN(theta=c(delta,beta1,beta2,phi),y=Y,X=X)
//' 
// [[Rcpp::export]]
...
//' @title Calculo do gradiente da log-posteriori dos parametros (\delta, \beta) sob o modelo Normal Reversa de Potencia
//' @name gradbetadeltaRPN
//'
//' @description Uma funcao que recebe os valores dos parametros \delta (parametro de forma) e coeficientes da regressao (\beta); a matriz de delineamento X e a matriz de respostas Y;
//' os valores dos efeitos aleatorios espaciais \phi_{is} e calcula o gradiente da log-posteriori sob o modelo probabilistico Normal Reversa de Potencia 
//' @usage ${gradbetadeltaRPN}(${theta,y, X})
//'
...
//' @param Y Uma matriz de respostas binarias
//'
//' @details The details of the gradbetadeltaRPN function can be found in the vignette. Users can access the vignette using \verb{vignette(package = "SpatialBinReg")}.
//'     
//'
//' @return Um vetor.
//'
...
//'
//'      
//'      gradbetadelatRPN(theta=c(delta,beta1,beta2,phi),y=Y,X=X)
//' 
//'
// [[Rcpp::export]]
    ...
    arma::colvec omegas;
       ...
       //  pis = 1-exp(-pow(zis2,(-1/xi)));
        pis = Fcpp(zis2,delta);
         ...
         omegas = (exp(delta)*normpdf(-zis2)/normcdf(-zis2))%((y.col(i)-pis)/pis); // s = 1, ...,m
...
//' @title Calculo da log-posteriori do vetor de efeitos aleatorios \phi_i, para i=1,...,n sob o modelo Normal Reversa de Potencia
//' @name lpostphiRPN
//'
...
//'     e calcula a log-posteriori para o vetor de efeitos aleatorios sob o modelo probabilistico Normal Reversa de Potencia 
//' @usage ${lpostphiRPN}(${phi,delta, bet, y, X, Omega})
//'
...
//'
//' @details The details of the lpostphiRPN function can be found in the vignette. Users can access the vignette using \verb{vignette(package = "SpatialBinReg")}.
//'     
//'
//' @return Um valor real.
//'
...
//'
//'      
//'      lpostphiRPN(phi=phi[1,],delta=delta,bet=c(beta1,beta2),y=Y[1,],X=X[1,])
//' 
//'
// [[Rcpp::export]]
...
//' @title Calculo do gradiente da log-posteriori do vetor de efeitos aleatorios \phi_i, para i=1,...,n sob o modelo Normal Reversa de Potencia
//' @name gradphiRPN
//'
//' @description Uma funcao que recebe os valores do vetor de efeitos aleatorios espaciais, dos parametros \delta (parametro de forma) 
//' e coeficientes da regressao (\beta); a matriz de delineamento X e a matriz de respostas Y; e matriz de precisao \Omega
//' e calcula o gradiente da log-posteriori sob o modelo probabilistico Normal Reversa de Potencia 
//' @usage ${gradphiRPN}(${phi,delta, bet, y, X, Omega})
//'
//' @param phi O vetor de efeitos aleatorios espaciais \phi_i , i=1,...,n 
//' @param delta O valor do parametro de forma \delta 
//' @param bet O vetor de coeficientes \beta
...
//' @param Omega Uma matriz referente à matriz de precisao
//'
//' @details The details of the gradphiRPN function can be found in the vignette. Users can access the vignette using \verb{vignette(package = "SpatialBinReg")}.
//'     
//'
//' @return Um vetor.
//'
...
//'
//'      
//'      gradphiRPN(phi=phi[1,],delta=delta,bet=c(beta1,beta2),y=Y[1,],X=X[1,])
//' 
// [[Rcpp::export]]
...
//' e dos coeficientes de regressao \beta sob o modelo Normal Reversa de Potencia
//' @name hmcRPN
//'
...
//' o modelo Normal Reversa de Potencia
//' @usage ${hmcRPN}(${theta_current, SS,burn,lag,epsilon,LF, epsphi, Lphi M, y, X, Omega})
//'
...
//'
//' @details The details of the hmcRPN function can be found in the vignette. Users can access the vignette using \verb{vignette(package = "SpatialBinReg")}.
//'     
//'
//' @return Um vetor.
//'
...
//' map = optim(para, lpostbetadeltaRPN, gradbetadeltaRPN,phi, y = Y, X=X, control = list(fnscale = -1), method = 'BFGS', hessian = TRUE); map
//'
//'
//' G. = Grpn(theta=c(round(map$par,2),phi),y=Y,X=X)
//'
...
//' Omegacpp=Omega_sim
//'
//'  B=hmcRPN(theta[i-1,],SS=1,burn=1,lag=1, epsilon=0.1, LF=22,epsphi = 0.001, Lphi = 22, M=G., y=Y,X=X, Omega=Omegacpp)
//' 
// [[Rcpp::export]]

...
//'     e calcula o valor da funcao de distribuicao acumulada do modelo probabilistico Gumbel Reversa (funcao de ligacao Cloglog)
//'
//' @usage ${Fcloglog}(${x})
  ...
  // verossimilhan�a     
       ...
       for (int j=0; j<m; j++){
		    ...
		    log_lik[i] = sum(y.row(i)%log(Fcpp(zis).t()))-
                        sum((1-y.row(i))%exp(zis).t());
                        
        //arma::vec loglik = {log_lik[i],0};
...
//'
//'      
//'      gradbetadelatCloglog(theta=c(beta1,beta2),phiinit=phi,y=Y,X=X)
//' 
// [[Rcpp::export]]
...
arma::vec gradbetaCpp(arma::vec theta,arma::mat y,arma::mat X) {
    ...
    arma::colvec omegas;
        ...
        for (int i=0; i<m; i++)
       {
         ...
         omegas = ((y.col(i)-pis)/pis)%exp(zis); // s = 1, ...,m
...
//' @title Calculo da log-posteriori do vetor de efeitos aleatorios \phi_i, para i=1,...,n sob a funcao de ligacao Cloglog
//' @name lpostphiCloglog
//'
...
//'     e calcula a log-posteriori para o vetor de efeitos aleatorios sob o modelo com funcao de ligacao Cloglog
//' @usage ${lpostphiCloglog}(${phi, bet, y, X, Omega})
//'
...
//'
//' @details The details of the lpostphiCloglog function can be found in the vignette. Users can access the vignette using \verb{vignette(package = "SpatialBinReg")}.
//'     
//'
//' @return Um valor real.
//'
...
//'
//'      
//'      lpostphiCloglog(phi=phi[1,],bet=c(beta1,beta2),y=Y[1,],X=X[1,])
//' 
// [[Rcpp::export]]
       ...
       for (int j=0; j<m; j++){
        ...
        //    Rcout << "zis:\n"   << zis   << std::endl;
		 ...
		 double lpostphi = sum(y%log(Fcpp(zis)))- 
                        sum((1-y)%exp(zis))-0.5*arma::as_scalar(phi.t()*Omega*phi);
...
//' @title Calculo do gradiente da log-posteriori do vetor de efeitos aleatorios \phi_i, para i=1,...,n para o modelo sob funcao de ligacao Cloglog
//' @name gradphiCloglog
//'
...
//' e calcula o gradiente da log-posteriori sob o modelo com funcao de ligacao Cloglog 
//' @usage ${gradphiCloglog}(${phi, bet, y, X, Omega})
//'
//' @param phi O vetor de efeitos aleatorios espaciais \phi_i , i=1,...,n 
//' @param bet O vetor de coeficientes \beta
//' @param y Um vetor com respostas binarias para a unidade observacional i, para i=1,...,n
...
//' @param Omega Uma matriz referente à matriz de precisao
//'
//' @details The details of the gradphiCloglog function can be found in the vignette. Users can access the vignette using \verb{vignette(package = "SpatialBinReg")}.
//'     
//'
//' @return Um vetor.
//'
...
//'
//'      
//'      gradphiCloglog(phi=phi[1,],bet=c(beta1,beta2),y=Y[1,],X=X[1,])
//' 
// [[Rcpp::export]]
...
//' e dos coeficientes de regressao \beta para o modelo sob funcao de ligacao Cloglog
//' @name hmcCloglog
//'
...
//' para o modelo sob funcao de ligacao Cloglog
//' @usage ${hmcCloglog}(${theta_current, SS,burn,lag,epsilon,LF, epsphi, Lphi M, y, X, Omega})
//'
...
//'
//' @details The details of the hmcCloglog function can be found in the vignette. Users can access the vignette using \verb{vignette(package = "SpatialBinReg")}.
//'     
//'
//' @return Um vetor.
//'
...
//' map = optim(para, lpostbetadeltaCloglog, gradbetadeltaCloglog,phi, y = Y, X=X, control = list(fnscale = -1), method = 'BFGS', hessian = TRUE); map
//'
//'
//' G. = Gcloglog(theta=c(round(map$par,2),phi),y=Y,X=X)
//'
...
//' Omegacpp=Omega_sim
//'
//'  B=hmcCloglog(theta[i-1,],SS=1,burn=1,lag=1, epsilon=0.1, LF=22,epsphi = 0.001, Lphi = 22, M=G., y=Y,X=X, Omega=Omegacpp)
//' 
// [[Rcpp::export]]

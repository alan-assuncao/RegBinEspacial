...
//' @title Calculo da funcao de distribuicao acumulada da distribuicao Cauchy Reversa de Potencia
//' @name Frpc
//'
...
//'
//' @details The details of the Frpc function can be found in the vignette. Users can access the vignette using \verb{vignette(package = "SpatialBinReg")}.
//'
...
arma::vec Fcpp (arma::vec x, double delta) { 
   ...
   double pi = 3.141593;
     ...
     z = 1- pow(((1/pi)*atan(-x)+0.5),exp(delta));
      
    return z;    
}

//' @title Calculo da log-posteriori dos parametros \delta e \beta sob o modelo Cauchy Reversa de Potencia
//' @name lpostbetadeltaRPC
//'
...
//'     e calcula a log-posteriori para os parametros \delta e \beta sob o modelo probabilistico Cauchy Reversa de Potencia 
//' @usage ${lpostbetadeltaRPC}(${theta,y, X})
//'
...
//'
//' @details The details of the lpostbetadeltaRPC function can be found in the vignette. Users can access the vignette using \verb{vignette(package = "SpatialBinReg")}.
//'     
//'
...
//'        
//'        p = Frpc((betax[k]+phi[k,]),delta)
//'        
//'        Y[k,] = rbinom(m, size=1, prob=p)
//'      }
//'
//'      
//'      lpostbetadelatRPC(theta=c(delta,beta1,beta2,phi),y=Y,X=X)
//' 
// [[Rcpp::export]]
...
//' @title Calculo do gradiente da log-posteriori dos parametros (\delta, \beta) sob o modelo Cauchy Reversa de Potencia
//' @name gradbetadeltaRPC
//'
//' @description Uma funcao que recebe os valores dos parametros \delta (parametro de forma) e coeficientes da regressao (\beta); a matriz de delineamento X e a matriz de respostas Y;
//' os valores dos efeitos aleatorios espaciais \phi_{is} e calcula o gradiente da log-posteriori sob o modelo probabilistico Cauchy Reversa de Potencia 
//' @usage ${gradbetadeltaRPC}(${theta,y, X})
//'
...
//' @param Y Uma matriz de respostas binarias
//'
//' @details The details of the gradbetadeltaRPC function can be found in the vignette. Users can access the vignette using \verb{vignette(package = "SpatialBinReg")}.
//'     
//'
//' @return Um vetor.
//'
...
//'
//'      
//'      gradbetadelatRPC(theta=c(delta,beta1,beta2,phi),y=Y,X=X)
//' 
//'
// [[Rcpp::export]]
...
arma::vec gradbetadeltaCpp(arma::vec theta,arma::mat y,arma::mat X) {
  ...
  double pi = 3.141593;
       ...
       {             
           ...
           deltais = exp(delta)*(log(2*pi)-log(2*atan(-zis)+pi));
    ...
    arma::colvec omegas;
       ...
       //  pis = 1-exp(-pow(zis2,(-1/xi)));
        pis = Fcpp(zis2,delta);
         ...
         omegas = (2*exp(delta)/((2*atan(-zis2)+pi)%(1+pow(zis2,2))))%((y.col(i)-pis)/pis); // s = 1, ...,m
...
//' @title Calculo da log-posteriori do vetor de efeitos aleatorios \phi_i, para i=1,...,n sob o modelo Cauchy Reversa de Potencia
//' @name lpostphiRPC
//'
...
//'     e calcula a log-posteriori para o vetor de efeitos aleatorios sob o modelo probabilistico Cauchy Reversa de Potencia 
//' @usage ${lpostphiRPC}(${phi,delta, bet, y, X, Omega})
//'
...
//'
//' @details The details of the lpostphiRPC function can be found in the vignette. Users can access the vignette using \verb{vignette(package = "SpatialBinReg")}.
//'     
//'
//' @return Um valor real.
//'
...
//'
//'      
//'      lpostphiRPC(phi=phi[1,],delta=delta,bet=c(beta1,beta2),y=Y[1,],X=X[1,])
//' 
// [[Rcpp::export]]
...
//' @title Calculo do gradiente da log-posteriori do vetor de efeitos aleatorios \phi_i, para i=1,...,n sob o modelo Cauchy Reversa de Potencia
//' @name gradphiRPC
//'
//' @description Uma funcao que recebe os valores do vetor de efeitos aleatorios espaciais, dos parametros \delta (parametro de forma) 
//' e coeficientes da regressao (\beta); a matriz de delineamento X e a matriz de respostas Y; e matriz de precisao \Omega
//' e calcula o gradiente da log-posteriori sob o modelo probabilistico Cauchy Reversa de Potencia 
//' @usage ${gradphiRPC}(${phi,delta, bet, y, X, Omega})
//'
//' @param phi O vetor de efeitos aleatorios espaciais \phi_i , i=1,...,n 
//' @param delta O valor do parametro de forma \delta 
//' @param bet O vetor de coeficientes \beta
...
//' @param Omega Uma matriz referente à matriz de precisao
//'
//' @details The details of the gradphiRPC function can be found in the vignette. Users can access the vignette using \verb{vignette(package = "SpatialBinReg")}.
//'     
//'
//' @return Um vetor.
//'
...
//'
//'      
//'      gradphiRPC(phi=phi[1,],delta=delta,bet=c(beta1,beta2),y=Y[1,],X=X[1,])
//' 
// [[Rcpp::export]]
...
arma::vec gradphiCpp(arma::vec phi, double delta, arma::vec bet, arma::vec y, arma::vec X, arma::mat Omega) {
  ...
  double pi = 3.141593;
        ...
        for (int j=0; j<m; j++){
         ...
         dzis = (2*exp(delta)/((2*atan(-zis)+pi)%(1+pow(zis,2))));
...
//' e dos coeficientes de regressao \beta sob o modelo Cauchy Reversa de Potencia
//' @name hmcRPC
//'
...
//' o modelo Cauchy Reversa de Potencia
//' @usage ${hmcRPC}(${theta_current, SS,burn,lag,epsilon,LF, epsphi, Lphi M, y, X, Omega})
//'
...
//'
//' @details The details of the hmcRPC function can be found in the vignette. Users can access the vignette using \verb{vignette(package = "SpatialBinReg")}.
//'     
//'
//' @return Um vetor.
//'
...
//' map = optim(para, lpostbetadeltaRPC, gradbetadeltaRPC,phi, y = Y, X=X, control = list(fnscale = -1), method = 'BFGS', hessian = TRUE); map
//'
//'
//' G. = Grpc(theta=c(round(map$par,2),phi),y=Y,X=X)
//'
...
//' Omegacpp=Omega_sim
//'
//'  B=hmcRPC(theta[i-1,],SS=1,burn=1,lag=1, epsilon=0.1, LF=22,epsphi = 0.001, Lphi = 22, M=G., y=Y,X=X, Omega=Omegacpp)
//' 
// [[Rcpp::export]]

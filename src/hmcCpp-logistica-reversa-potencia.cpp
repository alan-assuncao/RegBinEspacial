...
//' @title Calculo da funcao de distribuicao acumulada da distribuicao Logistica Reversa de Potencia
//' @name Frpl
//'
//' @description Uma funcao que recebe dois valores, o valor de x e o valor do parametro de forma delta
//'     e calcula o valor da funcao de distribuicao acumulada para o modelo probabilistico Logistica Reversa de Potencia 
//'
//' @usage ${Frpl}(${x, delta})
...
//'
//' @details The details of the Frpl function can be found in the vignette. Users can access the vignette using \verb{vignette(package = "SpatialBinReg")}.
//'
...
arma::vec Fcpp (arma::vec x, double delta) { 
      ...
      int n = x.n_elem;
      arma::vec z(n);
        z = 1-pow((1/(1+exp(x))),exp(delta));
      
    return z;    
}

//' @title Calculo da log-posteriori dos parametros \delta e \beta sob o modelo Logistica Reversa de Potencia
//' @name lpostbetadeltaRPL
//'
...
//'     e calcula a log-posteriori para os parametros \delta e \beta sob o modelo probabilistico Logistica Reversa de Potencia 
//' @usage ${lpostbetadeltaRPL}(${theta,y, X})
//'
...
//'
//' @details The details of the lpostbetadeltaRPL function can be found in the vignette. Users can access the vignette using \verb{vignette(package = "SpatialBinReg")}.
//'     
//'
...
//'        
//'        p = Frpl((betax[k]+phi[k,]),delta)
//'        
//'        Y[k,] = rbinom(m, size=1, prob=p)
//'      }
//'
//'      
//'      lpostbetadelatRPL(theta=c(delta,beta1,beta2,phi),y=Y,X=X)
//' 
// [[Rcpp::export]]
...
//' @title Calculo do gradiente da log-posteriori dos parametros (\delta, \beta) sob o modelo Logistica Reversa de Potencia
//' @name gradbetadeltaRPL
//'
//' @description Uma funcao que recebe os valores dos parametros \delta (parametro de forma) e coeficientes da regressao (\beta); a matriz de delineamento X e a matriz de respostas Y;
//' os valores dos efeitos aleatorios espaciais \phi_{is} e calcula o gradiente da log-posteriori sob o modelo probabilistico Logistica Reversa de Potencia 
//' @usage ${gradbetadeltaRPL}(${theta,y, X})
//'
...
//' @param Y Uma matriz de respostas binarias
//'
//' @details The details of the gradbetadeltaRPL function can be found in the vignette. Users can access the vignette using \verb{vignette(package = "SpatialBinReg")}.
//'     
//'
//' @return Um vetor.
//'
...
//'
//'      
//'      gradbetadelatRPL(theta=c(delta,beta1,beta2,phi),y=Y,X=X)
//' 
//'
// [[Rcpp::export]]
...
arma::vec gradbetadeltaCpp(arma::vec theta,arma::mat y,arma::mat X) {
  ...
  arma::vec phisoma(m);
       ...
       {             
           ...
           deltais = (exp(delta)*log(1+exp(zis)))/(pow((1+exp(zis)),exp(delta))-1);
    ...
    arma::colvec omegas;
       ...
       //  pis = 1-exp(-pow(zis2,(-1/xi)));
        pis = Fcpp(zis2,delta);
         ...
         omegas =  (exp(delta+zis2)/((pow((1+exp(zis2)),exp(delta))-1)%(1+exp(zis2))))%((y.col(i)-pis)/(1-pis)); // s = 1, ...,m
...
//' @title Calculo da log-posteriori do vetor de efeitos aleatorios \phi_i, para i=1,...,n sob o modelo Logistica Reversa de Potencia
//' @name lpostphiRPL
//'
...
//'     e calcula a log-posteriori para o vetor de efeitos aleatorios sob o modelo probabilistico Logistica Reversa de Potencia 
//' @usage ${lpostphiRPL}(${phi,delta, bet, y, X, Omega})
//'
...
//'
//' @details The details of the lpostphiRPL function can be found in the vignette. Users can access the vignette using \verb{vignette(package = "SpatialBinReg")}.
//'     
//'
//' @return Um valor real.
//'
...
//'
//'      
//'      lpostphiRPL(phi=phi[1,],delta=delta,bet=c(beta1,beta2),y=Y[1,],X=X[1,])
//' 
//'
// [[Rcpp::export]]
...
//' @title Calculo do gradiente da log-posteriori do vetor de efeitos aleatorios \phi_i, para i=1,...,n sob o modelo Logistica Reversa de Potencia
//' @name gradphiRPL
//'
//' @description Uma funcao que recebe os valores do vetor de efeitos aleatorios espaciais, dos parametros \delta (parametro de forma) 
//' e coeficientes da regressao (\beta); a matriz de delineamento X e a matriz de respostas Y; e matriz de precisao \Omega
//' e calcula o gradiente da log-posteriori sob o modelo probabilistico Logistica Reversa de Potencia 
//' @usage ${gradphiRPL}(${phi,delta, bet, y, X, Omega})
//'
//' @param phi O vetor de efeitos aleatorios espaciais \phi_i , i=1,...,n 
//' @param delta O valor do parametro de forma \delta 
//' @param bet O vetor de coeficientes \beta
...
//' @param Omega Uma matriz referente à matriz de precisao
//'
//' @details The details of the gradphiRPL function can be found in the vignette. Users can access the vignette using \verb{vignette(package = "SpatialBinReg")}.
//'     
//'
//' @return Um vetor.
//'
...
//'
//'      
//'      gradphiRPL(phi=phi[1,],delta=delta,bet=c(beta1,beta2),y=Y[1,],X=X[1,])
//' 
// [[Rcpp::export]]
...
arma::vec gradphiCpp(arma::vec phi, double delta, arma::vec bet, arma::vec y, arma::vec X, arma::mat Omega) {
  ...
  arma::vec D;
        ...
        for (int j=0; j<m; j++){
         ...
         dzis = exp(delta+zis)/((pow((1+exp(zis)),exp(delta))-1)%(1+exp(zis)));
...
//' e dos coeficientes de regressao \beta sob o modelo Logistica Reversa de Potencia
//' @name hmcRPL
//'
...
//' o modelo Logistica Reversa de Potencia
//' @usage ${hmcRPL}(${theta_current, SS,burn,lag,epsilon,LF, epsphi, Lphi M, y, X, Omega})
//'
...
//'
//' @details The details of the hmcRPL function can be found in the vignette. Users can access the vignette using \verb{vignette(package = "SpatialBinReg")}.
//'     
//'
//' @return Um vetor.
//'
...
//' map = optim(para, lpostbetadeltaRPL, gradbetadeltaRPL,phi, y = Y, X=X, control = list(fnscale = -1), method = 'BFGS', hessian = TRUE); map
//'
//'
//' G. = Grpl(theta=c(round(map$par,2),phi),y=Y,X=X)
//'
...
//' Omegacpp=Omega_sim
//'
//'  B=hmcRPL(theta[i-1,],SS=1,burn=1,lag=1, epsilon=0.1, LF=22,epsphi = 0.001, Lphi = 22, M=G., y=Y,X=X, Omega=Omegacpp)
//' 
// [[Rcpp::export]]
